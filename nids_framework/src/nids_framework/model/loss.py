...
class HybridReconstructionLoss(nn.Module):
    __slots__ = ["mse_loss", "ce_loss", "lambda_num", "lambda_cat", "categorical_levels"]

    def __init__(self, lambda_num: float = 1.0, lambda_cat: float = 1.0, categorical_levels: int = 32):
        super().__init__()
    ...
    def forward(
        self,
        num_recon: torch.Tensor, num_target: torch.Tensor,
        cat_recon: torch.Tensor, cat_target: torch.Tensor
    ) -> torch.Tensor:
