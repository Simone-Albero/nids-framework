...
def transform_wrapper(transform_function):
    class TransformClass:
        ...
        def __call__(self, sample: Any):
            return self.transform_function(sample, *self.args, **self.kwargs)
...
def bound_transformation(sample: Any, bound: int):
    ...
    features = torch.where((features < -bound) | (features > bound), torch.tensor(0.0), features)
...
def one_hot_transformation(sample: Any, categorical_bound: int):
    ...
    for i, column in enumerate(columns):
        ...
        sorted_unique_values = list(sorted(zip(unique_values, counts), key=lambda x: x[1], reverse=True))
