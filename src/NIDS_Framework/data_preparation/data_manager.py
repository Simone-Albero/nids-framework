...
class DataManager:
    ...
    __slots__ = ['_train_df', '_test_df', '_properties', '_numeric_transformations', '_categorical_transformations']
    ...
    def _load_df(self, dataset_path: str) -> pd.DataFrame:
        ...
        if not os.path.exists(dataset_path):
            raise FileNotFoundError(f'The dataset file was not found at the specified path: {dataset_path}')
    ...
    def train_data(self) -> custom_dataset.CustomDataset:
        ...
        dataset = custom_dataset.CustomDataset(self._train_df[self._properties.numeric_features], self._train_df[self._properties.categorical_features], self._train_df[self._properties.labels], Compose(self._numeric_transformations), Compose(self._categorical_transformations))
        return dataset
    ...
    def add_transformation(self, transformation: Callable, is_categorical: bool = False) -> None:
        if not is_categorical: self._numeric_transformations.append(transformation)
        else: self._categorical_transformations.append(transformation)