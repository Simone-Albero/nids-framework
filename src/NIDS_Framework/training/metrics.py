...
class Metric():
    ...
    def step(self, y: torch.Tensor, y_true: torch.Tensor) -> None:
        ...
        batch_confusion_matrix = confusion_matrix(y_true.cpu().numpy(), y_pred.cpu().numpy(), labels=np.arange(self._n_classes))
    ...
    def save(self, file_path: Optional[str] = "logs/metrics.csv", reset_logs: Optional[bool] = False) -> None:
...
class MulticlassClassificationMetric(Metric):
    ...
    def __init__(self, n_classes: int) -> None:
        ...
        self._class_stats: NDArray = np.zeros((n_classes, 3))  # Stores precision, recall, f1 for each class
    ...
    def __str__(self) -> str:
        ...
        for i in range(self._n_classes):
            ...
            output.append(f"Class {i} - Precision: {precision:.4f}, Recall: {recall:.4f}, F1 Score: {f1:.4f}\n")
    ...
    def _write_metrics(self, file_handle) -> None:
        ...
        file_handle.write(f"Weighted,{self.weighted_precision:.4f},{self.weighted_recall:.4f},{self.weighted_f1:.4f},{total_support}\n")